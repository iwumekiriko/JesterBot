...
async def get_cfg(guild_id: int, cfg_type: type[T]) -> T:
    async with aiohttp.ClientSession() as session:
        async with session.get(
            PATH_TO_API + f"Config/{cfg_type(0).short_name}/{guild_id}", ssl=False
        ) as response:
            if response.status == 200:
                ...
                raise BaseException(f"Config API [{cfg_type(0).short_name}] is not responding. "
...
async def set_cfg(cfg: T) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.put(
            PATH_TO_API + f"Config/{cfg.short_name}",
                ssl=False, data=data, headers=headers
        ) as response:
            if not response.status == 200:
                error_message = await response.text()
                raise BaseException(f"Config API [{cfg.__class__.__name__}] is not responding. "
                                    f"Status code: {response.status}. Error: {error_message}")
