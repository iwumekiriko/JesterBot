        ...
        async with session.post(
            ...
            data=data, headers=headers, ssl=False
                ...
                if response.status == 200:
                    ...
                    raise CustomException("Tickets API [Create] is not responding. "
...
async def _ticket_get(ticket_id: int) -> Ticket:
    async with aiohttp.ClientSession() as session:
        async with session.get(
            PATH_TO_API + f"Tickets/{ticket_id}", ssl=False
        ) as response:
            if response.status == 200:
                ...
                error_message = await response.text()
                raise CustomException("Tickets API [Get] is not responding. "
                                    f"Status code: {response.status}. Error: {error_message}")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.put(
            ...
            if response.status == 200:
                ...
                error_message = await response.text()
                raise CustomException("Tickets API [Start] is not responding. "
                                    f"Status code: {response.status}. Error: {error_message}")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.put(
            ...
            if response.status == 400:
                ...
                error_message = await response.text()
                raise CustomException("Tickets API [Close] is not responding. "
                                    f"Status code: {response.status}. Error: {error_message}")
